...
const Approve = () => {
  ...
  const fetchAllOTRequests = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/user/getAllOTRequests`, {
        method: 'GET',
        headers: {
  ...
  const handleApprove = async (otId) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/user/approveOTRequest`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
